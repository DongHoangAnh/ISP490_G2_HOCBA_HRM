...
| **M-10** | **Tách tài khoản quản lý ↔ cá nhân:** màn quản lý chỉ hiển thị menu quản lý; nhân viên thường chỉ thấy luồng cá nhân ("Hồ sơ của tôi"); vai trò gắn theo group Odoo (bàn giao được). **(Bổ sung 2026-06-20)** quy tắc tách áp dụng cho **cả Trưởng phòng** ("HR nhỏ" phòng ban) — `isRoleAccount` gồm `isManager`; người giữ vai trò dùng tài khoản nhân viên riêng cho hồ sơ cá nhân. | Bổ sung nav theo vai trò trong SPA + endpoint `/hocba-hrm/api/me/roles`. **Quản lý** phân quyền theo **phòng ban** (`_managed_department_ids`, gồm phòng con). | ✅ Implement (nav vai trò + scope phòng ban + Trưởng phòng ẩn cá nhân 2026-06-20; panel chấm công self thuộc `hocba_attendance` — cross-team) |
