/* Quản lý báo cáo BHXH — Owner: Hùng. */
import { useState, useEffect } from 'react';
import { fetchBhxhReports, fetchBatches, createBhxhReport, computeBhxh, submitBhxh } from '../../api/payroll';
...
import BhxhDrawer from './BhxhDrawer';
...
export default function BhxhReport() {
  ...
  const load = () => {
    ...
    fetchBhxhReports().then(setData).catch((e) => setErr(e.message));
  };
  useEffect(load, []);
  ...
  if (!data) return <LoadingState label="Đang tải báo cáo BHXH..." />;
      ...
      <div className="card">
        {data.length === 0 ? (
          <div style={{ padding: 36, textAlign: 'center' }}>
            <EmptyState>Chưa có báo cáo BHXH.</EmptyState>
          </div>
        ) : (
            ...
            <table className="tbl">
              <thead>
                <tr>
                  ...
                  <th style={{ textAlign: 'right' }}>BHYT (NV)</th>
                  <th style={{ textAlign: 'right' }}>BHTN (NV)</th>
                  <th style={{ textAlign: 'right' }}>BHXH (DN)</th>
                  <th>Trạng thái</th>
                ...
                {data.map((r) => {
                  ...
                  return (
                    <tr key={r.id} style={{ cursor: 'pointer' }} onClick={() => setSel(r)}>
                      ...
                      <td style={{ textAlign: 'right' }}>{hbVND(r.total_bhxh_ee)}</td>
                      <td style={{ textAlign: 'right' }}>{hbVND(r.total_bhyt_ee)}</td>
                      <td style={{ textAlign: 'right' }}>{hbVND(r.total_bhtn_ee)}</td>
                      <td style={{ textAlign: 'right' }}>{hbVND(r.total_bhxh_er)}</td>
                      <td><Badge kind={sk}>{sl}</Badge></td>
                      <td>
                        <div style={{ display: 'flex', gap: 6 }} onClick={(e) => e.stopPropagation()}>
                          {r.state === 'draft' && (
                            <button className="btn btn-ghost btn-sm"
                              onClick={() => doAction(r.id, computeBhxh, 'Tính BHXH')}
                              disabled={busy === r.id}>
                          ...
                          {r.state === 'computed' && (
                            <button className="btn btn-ghost btn-sm"
                              onClick={() => doAction(r.id, submitBhxh, 'Nộp BHXH')}
                              disabled={busy === r.id}>
...
/* ── Inline form — tạo báo cáo BHXH ── */
function BhxhForm({ onClose, onSaved }) {
  ...
  return (
    <Modal onClose={onClose}>
      <div className="drawer-head" style={{ background: 'linear-gradient(120deg,var(--red-50),#fff)' }}>
        <div style={{ flex: 1 }}>
          <h2 style={{ margin: 0, fontSize: 19, fontWeight: 800 }}>Tạo báo cáo BHXH</h2>
          <div className="muted" style={{ fontSize: 13, marginTop: 2 }}>Tạo báo cáo bảo hiểm xã hội tháng</div>
        </div>
